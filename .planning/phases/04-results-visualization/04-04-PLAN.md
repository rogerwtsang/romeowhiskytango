...
must_haves:
  truths:
    ...
    - "User can copy lineup candidate to main panel"
  artifacts:
...
NOTE: These widgets are UI-ready but won't have data until the optimizer is built. They should work with mock data for testing.
  </action>
  <verify>
```bash
# Type check
...
ranking = LineupRankingList(root)
diff = LineupDiffView(root)
print('Widgets created OK')
root.destroy()
"
```
  </verify>
...
5. Verify in Results panel:
   ...
   - Win probability displayed with CI
   - LOB/RISP metrics displayed (or "--" if unavailable)
   - Player contribution chart visible in details section
   - Toggle between slot and player view works
...
- Copy-to-panel callback wired correctly
